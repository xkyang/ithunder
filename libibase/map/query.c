...
*/	
        ...
        if(query->int_inset_count > 0)
        {
            for(i = 0; i < query->int_inset_count; i++)
            {
				...
				fprintf(stdout,"INT>>>i:%d field_id:%d k:%d secid:%d inset_num:%d value:%d n:%d\n",i,query->int_inset_list[i].field_id,k,secid,inset_num,
						query->int_inset_list[i].set[0],n);
				fprintf(stdout,"################%d begin###################\n",k);
                docs_size = (n + 100) * sizeof(uint32_t);
                docs = (uint32_t *)mdb_new_data(PMDB(ibase->index), docs_size);
				if(docs)
				{
	               n = imap_ins(ibase->state->mfields[secid][k], query->int_inset_list[i].set, inset_num, docs);
				   off = 0;
                   while(docs && off < n)
                   {
                     docid = docs[off];
                   ...
                   mdb_free_data(PMDB(ibase->index), (char *)docs, docs_size);
				}
				else
				{
				   fprintf(stdout,"INT>>>mdb_new_data failed!\n");
		           exit(-1);		
				}
				fprintf(stdout,"###################################\n");
				if(0 == n) goto end;
        ...
        if(query->long_inset_count > 0)
        {
            for(i = 0; i < query->long_inset_count; i++)
            {
				...
				fprintf(stdout,"LONG>>>i:%d field_id:%d k:%d secid:%d inset_num:%d value:%ld n:%d\n",i,query->long_inset_list[i].field_id,k,secid,inset_num,
						query->long_inset_list[i].set[0],n);
				fprintf(stdout,"################%d begin###################\n",k);
                docs_size = (n + 100) * sizeof(uint32_t);
                docs = (uint32_t *)mdb_new_data(PMDB(ibase->index), docs_size);
				if(docs)
				{
	              n = lmap_ins(ibase->state->mfields[secid][k], query->long_inset_list[i].set, inset_num, docs);
				  off = 0;
                  while(docs && off < n)
                  {
                     docid = docs[off];
				   ...
				   if(off == n)
				   {
		              docid = 777074;
                      xlong = LMAP_GET(ibase->state->mfields[secid][k], docid);
                      fprintf(stdout, "\tFAILED----->docid:%d/%lld off:%d value:%ld k:%d\n", docid, LL(headers[docid].globalid), off, xlong, k);
				   }
                  mdb_free_data(PMDB(ibase->index), (char *)docs, docs_size);
				}
				else
				{
				   fprintf(stdout,"LONG>>>mdb_new_data failed!\n");
				   exit(1);
				}
				fprintf(stdout,"###################################\n");
				if(0 == n) goto end;
	...
	fprintf(stdout,"INT>>>i:%d field_id:%d k:%d secid:%d ifrom:%d ito:%d n:%d\n",i,query->int_range_list[i].field_id,k,secid,ifrom,ito,n);
				fprintf(stdout,"################%d begin###################\n",k);
                docs_size = (n + 100) * sizeof(uint32_t);
                docs = (uint32_t *)mdb_new_data(PMDB(ibase->index), docs_size);
				if(docs)
				{
	              n = imap_range(ibase->state->mfields[secid][k], ifrom, ito, docs);
				  off = 0;
                  while(docs && off < n)
                  {
                   docid = docs[off];
				...
				{
				   fprintf(stdout,"INT-range>>>mdb_new_data failed!\n");
				   exit(1);
				}
				fprintf(stdout,"###################################\n");
				        if(0 == n) goto end;
	...
	fprintf(stdout,"LONG>>>i:%d field_id:%d k:%d secid:%d lfrom:%ld lto:%ld n:%d\n",i,query->int_range_list[i].field_id,k,secid,lfrom,lto,n);
				fprintf(stdout,"################%d begin###################\n",k);
                docs_size = (n + 100) * sizeof(uint32_t);
                docs = (uint32_t *)mdb_new_data(PMDB(ibase->index), docs_size);
				if(docs)
				{
	               n = lmap_range(ibase->state->mfields[secid][k], lfrom, lto, docs);
				   off = 0;
                   while(docs && off < n)
                   {
                   docid = docs[off];
				...
				{
				   fprintf(stdout,"LONG-range>>>mdb_new_data failed!\n");
				   exit(1);
				}
				fprintf(stdout,"###################################\n");
				        if(0 == n) goto end;
	...
	fprintf(stdout,"RESULT>>>min_set_fid:%d min_set_num:%d secid:%d\n",min_set_fid,min_set_num,secid);
        if(min_set_num > 0)
        {
	...
	fprintf(stdout,"RESULT>>>min_set_fid:%d min_set_num:%d secid:%d ndocs:%d\n",min_set_fid,min_set_num,secid,ndocs);
    fprintf(stdout, "int[%d/%d] long[%d/%d]\n", query->int_range_count, query->int_inset_count, query->long_range_count,query->long_inset_count);
        TIMER_SAMPLE(timer);
...
*/	    
	    if(expr)
	    {
            ...
            res->total++;
            fprintf(stdout, "docid:%d/%lld base_socre:%lld total:%d\n", docid, LL(headers[docid].globalid), IBLL(base_score),res->total);
            ++off;
            continue;
next:
            docs[off++] = 0;
