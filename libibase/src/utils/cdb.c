...
void cdb_push_mblock(CDB *db, char *mblock, int block_index)
{
    ...
    if(db && mblock && block_index >= 0 && block_index < CDB_XCBLOCKS_MAX)
    {
        ...
        {
            DEBUG_LOGGER(db->logger, "free-xblock[%d]{%d}->total:%d", block_index, cdb_xblock_list[block_index].block_size, db->xblocks[block_index].total);
...
int cdb_pop_block(CDB *db, int blocks_count, XLNK *lnk)
{
    ...
    if(db && (x = (blocks_count - 1)) >= 0 && lnk)
    {
        ...
        if(block_id >= 0)
        {
            DEBUG_LOGGER(db->logger, "push_block() blockid:%d index:%d block_size:%d", block_id, cdb_id, block_size);
...
int cdb__set__data(CDB *db, int id, char *data, int ndata)
{
            ...
            && db->status == 0 && (dbx = (CDBX *)(db->dbxio.map)))
    {
        ...
        if(old.count > 0)
        {
            DEBUG_LOGGER(db->logger, "push_block() dbxid:%d blockid:%d index:%d block_size:%d",id, old.blockid, old.index, old.count * CDB_BASE_SIZE);
...
int cdb__add__data(CDB *db, int id, char *data, int ndata)
{
            ...
            && ndata > 0 && (dbx = (CDBX *)(db->dbxio.map)))
    {
        ...
        if(old_lnk.count > 0)
        {
            ...
            DEBUG_LOGGER(db->logger, "push_block() blockid:%d index:%d block_size:%d", old_lnk.blockid, old_lnk.index, old_lnk.count * CDB_BASE_SIZE);
