    ...
    if(lmap && (n = lmap->state->count) > 0)
    {
        ...
        else
        {
            while(max > min)
            {
                ...
                else if(key == lmap->slots[x].min)
                {
                   if(key <= lmap->slots[x-1].max)
                   {
                      ret = max = x - 1;
                   }
                   else
                ...
                else//key < lmap->slots[x].min
                {
                   ret = max = x - 1;
                }
            }
            if(ret >= 0 && key > lmap->slots[ret].max && ret < (n - 1))
            {
               ++ret;
            }
			if((x = ret)>= 0 && x < n)
    ...
    if(lmap && (n = lmap->state->count) > 0)
    {
        ...
        else
        {
            while(max > min)
            {
                ...
                else if(key == lmap->slots[x].max)
                {
                   if(key < lmap->slots[x+1].min)
                   {
                      ret = x;
                      break;
                   }
                ...
                else//key > lmap->slots[x].max
                {
                   ret = min = x + 1;
                }
            }
            if(ret >= 0 && key < lmap->slots[ret].min)
            {
               --ret;
            }
			if((x = ret) >= 0 && x < n)
...
int main()
{
    ...
    if((lmap = lmap_init("/tmp/1.idx")))
    {
        ...
        */
            ...
            fprintf(stdout, "range():%d\n", lmap_range(lmap, 1111111, 1600000, NULL));
#endif
#ifdef TEST_RANGE
        srand(time(NULL));
        TIMER_RESET(timer);
