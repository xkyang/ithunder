...
next:
        ...
        ACCESS_LOGGER(ibase->logger, "bsort text qid:%d secid:%d documents res:%d/%d time used:%lld ioTime:%lld sortTime:%lld ncatgroups:%d ngroups:%d", query->qid, secid, res->total, res->count, PT_USEC_U(timer), IBLL(res->io_time), IBLL(res->sort_time),res->ncatgroups, res->ngroups);
end:
        //free db blocks
        if(itermlist)
        {
